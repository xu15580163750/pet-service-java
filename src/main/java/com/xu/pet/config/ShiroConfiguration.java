...
import com.xu.pet.filter.URLPathMatchingFilter;
import com.xu.pet.realm.PetRealm;
...
//        这个 authc 即 autentication，是 shiro 自带的过滤器。除了它以外，常用的还有 anon（可匿名访问）、roles（需要角色）、perms（需要权限）
    ...
    public SecurityManager securityManager() {
        ...
        securityManager.setRealm(getPetRealm());
    ...
    @Bean
    public PetRealm getPetRealm() {
        PetRealm petRealm = new PetRealm();
        petRealm.setCredentialsMatcher(hashedCredentialsMatcher());
        return petRealm;
    }
